...
void MacOSBluetoothConnector::connectToMac(MacOSBluetoothConnector *macOSBluetoothConnector,
    ...
    // try to open channel
    if ([device openRFCOMMChannelAsync:&channel
                         withChannelID:rfcommChannelID
                              delegate:asyncCommDelegate] != kIOReturnSuccess) {
    ...
    if (@available(macOS 12.0, *)) {
        ...
        macOSBluetoothConnector->disconnectionConditionVariable.wait(
            lk, [&] { return !macOSBluetoothConnector->running; });
        lk.unlock();
    } else {
...
void MacOSBluetoothConnector::connect(const std::string &addrStr) {
    ...
    NSString *addressNSString = [NSString stringWithCString:addrStr.c_str()
